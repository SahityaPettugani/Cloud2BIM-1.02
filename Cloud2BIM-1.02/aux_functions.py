...
def identify_walls(pointcloud, pointcloud_resolution, minimum_wall_length, minimum_wall_thickness,
                   ...
                   exterior_scan=False, exterior_walls_thickness=0.3, wall_density_threshold=0.001,
                   wall_z_section_boundaries=(0.2, 0.95)):
    x_coords, y_coords, z_coords = zip(*pointcloud)
    z_section_boundaries = list(wall_z_section_boundaries)  # percentage of the height for the storey sections
    ...
    threshold = wall_density_threshold  # relative point cloud density for sparse room scans
    ...
    if not contours:
        print("Warning: No wall contours found. Try lowering thresholds or increasing point density.")
        return ([], [], [], [], [], [])
    ...
    if not filtered_segments:
        print("Warning: No wall segments passed length filter.")
        return ([], [], [], [], [], [])
    ...
    if not parallel_groups:
        print("Warning: No parallel wall groups found.")
        return ([], [], [], [], [], [])
    ...
    if not wall_axes:
        print("Warning: No wall axes computed.")
        return ([], [], [], [], [], [])
...
def assign_points_to_walls(x_coords, y_coords, z_coords, wall_axes, parallel_groups, z_floor, z_ceiling,
    ...
    wall_thicknesses = np.array([compute_wall_thickness(group) for group in parallel_groups], dtype=np.float32)
    ...
    line_starts = np.array([axis[0] for axis in wall_axes], dtype=np.float32)
    line_ends = np.array([axis[1] for axis in wall_axes], dtype=np.float32)

    # Group points based on their closest wall, in batches to avoid huge allocations
    wall_groups = [[] for _ in range(len(wall_axes))]
    n_points = valid_points.shape[0]
    for start_idx in range(0, n_points, batch_size):
        end_idx = min(start_idx + batch_size, n_points)
        batch_points = valid_points[start_idx:end_idx]

        # Calculate distances for all points to each wall axis for this batch
        all_distances = np.array(
            [distance_points_to_line_np(batch_points[:, :2], start, end) for start, end in zip(line_starts, line_ends)],
            dtype=np.float32)
